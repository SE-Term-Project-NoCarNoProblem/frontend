...
function EditProfile() {
	...
	async function onSave() {
		...
		if (form.profilePic != null) {
			...
			if (!res2.ok)
				throw new Error(`POST /profile/upload failed: ${res2.status}`);
		}
    ...
    return (<>
        ...
        <div className="bg-white p-12 flex flex-col items-center text-[#000000] mt-0">
            ...
            <div className={`w-full max-w-5xl bg-[#0E4663] rounded-2xl shadow-md p-8 border-r-gray-400`}>
				...
				<button
					className={`bg-[#0E4663] mt-15 text-[#F5F5F5] min-w-fit max-w-56 w-1/4 p-5 rounded-xl`}
					onClick={onSave}
				>
					Confirm change
				</button>
