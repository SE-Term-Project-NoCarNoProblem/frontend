...
interface RatingStarsProps {
  rideId: string; // Pass this from the parent
}

export default function RatingStars({ rideId }: RatingStarsProps) {
  const Map = useMemo(
    () =>
...
const handleSubmit = async () => {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/drivers/${rideId}/rating`,
      {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({ rating }), // ✅ backend expects { rating }
      }
    );
  ...
  useEffect(() => {
    const fetchRide = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/requests/nearby`
        );
